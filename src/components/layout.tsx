...
function Layout({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="bg-white">
      <header className="w-full basis-full fixed top-0 bg-white border-b shadow-sm h-[64px]">
        <div className="lg:px-[32px] px-[16px] w-full flex justify-between">
          ...
          <button className="leading-[64px] font-semibold flex items-center">
            ...
            />
            <span className="pl-[8px] hidden lg:inline-block">로그아웃</span>
          </button>
        </div>
      </header>
      <aside
        className={`h-screen py-[16px] w-[250px] border-r shadow-sm bg-white fixed lg:top-[64px] duration-700 lg:duration-0 transition-all lg:left-0 ${
      ...
      >
        <nav className="px-[16px]">
          ...
          <ul className="pt-5 lg:pt-0 flex flex-col gap-[12px]">
            <li className="p-[8px] hover:bg-blue-400 rounded-md hover:text-white cursor-pointer">
              메뉴1
            </li>
            <li className="p-[8px] hover:bg-blue-400 rounded-md hover:text-white cursor-pointer">
              메뉴2
